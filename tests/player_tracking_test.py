...
def main():
    ...
    for video_index, video_path in enumerate(video_files):
        ...
        # Read frame size from the video for the writer
        ...
        # model.track uses ByteTrack internally via bytetrack.yaml
        results = model.track(
            source=str(video_path),
            classes=[0],               # people only (COCO class 0)
            stream=True,
            imgsz=416,                 # sz416 is the best speed/accuracy balance per testing
            conf=0.35,                 # confidence threshold
            iou=0.8,                   # high iou keeps best box when players overlap
        ...
        for result in results:
            ...
            # Draw ByteTrack boxes with persistent IDs
