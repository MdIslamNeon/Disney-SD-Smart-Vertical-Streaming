...
# ----------------------------- Pass 1: Detect ---------------------------------
def detect_crop_positions(yolo, ball_class_id: int, input_path: str) -> tuple[list, int, int, int]:
    """
    ...
    known_boxes = []    # box_xyxy for each known detection
    total_frames = 0
    ...
    # Build a per-frame box lookup (only frames that survived outlier rejection)
    frame_boxes = {idx: box for idx, box in zip(known_indices, known_boxes[:len(known_indices)])}
